...
class MentorView(viewsets.ModelViewSet):
    ...
    ordering = ['last_name']
    search_fields = ['^last_name', '^level', '^email']
    ...
    def list(self, request):
        ...
        # queryset = queryset.annotate(last_name=Collate('last_name', 'first_name'))
        queryset = queryset.distinct()
        filter_query_set = self.filter_queryset(queryset)
        # serializer = self.get_serializer(filter_query_set, many)
    ...
    post_response_schema = {
        ...
        status.HTTP_500_INTERNAL_SERVER_ERROR: openapi.Response(
            ...
            examples={
                "application/json": {
                    "error": ERROR_CREATING_MENTOR #if you get errors check here
                    }
                }
            )
        }
    ...
    update_response_schema = {
        ...
        status.HTTP_500_INTERNAL_SERVER_ERROR: openapi.Response(
            ...
            examples={
                "application/json": {
                    "error": ERROR_UPDATING_MENTOR #if you get errors check here
                }
            }
        )
    }
